...
<!-- 對抗式 End-user UX review pass：接續 #42–#48，以校正操作員視角收斂 GUI 使用邏輯與美感。 -->

### Fixed
- 校正流 UX 收斂（End-user 對抗式 review）：
  - 快捷鍵改為只在「校正」模式生效，掃描模式吞鍵不動作，避免掃描中誤按 `Enter`/`F2`/`PgDn`
    觸發寫入或換頁；另新增 `F4`＝鍵盤切換 掃描/校正、`F8`＝跳到姓名候選清單。
...
- (#46) 資料完整性守門：姓名待確認且為空時，「確認並寫入」會擋下並提示先填姓名（強制寫入仍可覆蓋），
  避免把空姓名寫入並悄悄清掉待確認旗標。
- (#47) 來源圖放大改從**全解析度原圖**重採樣可視區域（保留原圖、依縮放裁切原圖再縮放），取代原本放大
  已降採樣縮圖造成的鋸齒；手寫筆畫放大後更清楚。
- 載入新批次後自動切回「校正」模式；roster 無建議時顯示「（無建議名單）」而非空白清單。

### Changed
- 校正介面美感與可發現性：
  - 狀態徽章改為彩色（已寫入綠／被擋下紅／待處理灰）而非同色純文字；底部新增常駐「模式：校正/掃描」
    指示，切換鈕文字改標示「切換到…（F4）」。
  - 聚焦中的欄位標題改為粗體，鍵盤導覽時看得出當前欄位（與 ⚠ 紅字／淡灰分屬不同視覺通道）。
  - 工具列新增「快捷鍵」說明鈕（彈出完整快捷鍵列表），主要按鈕加上 hover 提示（含 確認 vs 強制寫入 差異）。
  - (#44/#47) 校正模式新增靜態來源圖的「放大／縮小／符合視窗」鈕（先前只有滑鼠滾輪可縮放），與相機縮放分流。
...
- (#44) 工具列拆「掃描/校正」兩模式：依「掃描/校正」鈕切換，校正模式只留上一筆/下一筆/確認並寫入/強制
  寫入（＋進度），掃描站功能（擷取並辨識/匯入資料夾批次/選擇攝影機/旋轉/放大/縮小）收進掃描模式，根治
  校正中誤點。
