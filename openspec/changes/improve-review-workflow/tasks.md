...
- [ ] 1.1 Fail-first tests for `review_workflow`: `correction_mode_controls()` / `scan_mode_controls()`
...
- [ ] 1.2 Implement `src/ocr_from2xlsx/review_workflow.py` with those pure functions (no Tk/cv2 imports).

**Quality Gate:** helper tests pass with no Tk.

## Phase 2: Workbook row-targeted overwrite

- [ ] 2.1 Fail-first `test_workbook` cases: `write_record(record, row=R)` writes to row `R` (not the next
...
- [ ] 2.2 Implement `WorkbookWriter.write_record(self, record, row: int | None = None)`: when `row` is None
...
- [ ] 3.1 Fail-first `test_session` cases: `accept_scan(record, overwrite_row=R, human_confirmed=True)`
...
- [ ] 3.2 Implement `ImportSession.accept_scan(..., overwrite_row: int | None = None)`: thread `overwrite_row`
...
- [ ] 4.1 Fail-first tests (real-Tk + headless): a mode toggle switches between Scan and Correction; in
...
- [ ] 4.2 Implement the toolbar split in `_build_ui`: build two control groups, show/hide by mode via the
...
- [ ] 5.1 Fail-first tests: a persistent label shows `已寫入 X / 共 N` + the current row; `_show_record`
  shows the per-record badge (已寫入/待處理/被擋下) via `record_badge_state`; navigating back to a written
  record shows 已寫入 + its row.
- [ ] 5.2 Implement the progress/row indicator + badge in `app.py`, tracking `record_index → written_row`
...
- [ ] 6.1 Fail-first tests: when a record has `ocr.name_crop`, the name panel shows that crop (zoomed),
...
- [ ] 6.2 Implement the name-crop panel + selectable roster list in `app.py`, reusing the preview-image
...
- [ ] 7.1 Fail-first tests (headless fakes): re-opening a written record and confirming overwrite calls
...
- [ ] 7.2 Implement the re-open/overwrite flow in `app.py`: detect a confirm on an already-written record,
...
- [ ] 8.1 CHANGELOG `[Unreleased]` entries for #44/#45/#46/#48; README correction-workflow notes (modes,
  progress/badges, name aids, overwrite). No new CLI subcommand → CLI help unchanged.
- [ ] 8.2 `python -W error -m pytest -q` and `python -m policy_check --repo .` green.
- [ ] 8.3 Manual verify on a real display: scan↔correction toggle; progress/badges; name crop + roster pick;
  re-open a written record and overwrite its row (confirm no duplicate). Record behavior in the PR.
- [ ] 8.4 Base OpenSpec spec (`openspec/specs/record-confirmation/spec.md`) synced on archive.

**Quality Gate:** full suite + policy green; docs synced; manual run recorded.

## Completion Checklist

- [ ] All phases complete and quality gates green
- [ ] CHANGELOG `[Unreleased]`, README, and PR-template checklist done
- [ ] Ready for `/openspec-archive improve-review-workflow`
