...
- [x] 1.1 Add fail-first tests for `review_nav` driven by plain data (no Tk): `next_flagged_key` /
...
- [x] 1.2 Implement `src/ocr_from2xlsx/review_nav.py`: pure `next_flagged_key(order, flagged, current)`,
  `prev_flagged_key(order, flagged, current)`, and `option_index_for_digit(char, option_count)`. No
  imports of Tk/cv2.

**Quality Gate:**
- [x] Helper tests pass with neither Tk nor a display.

## Phase 2: ConfirmForm focus / de-emphasis / count surface

- [x] 2.1 Add fail-first real-Tk tests (skip on `tk.TclError`) for `ConfirmForm`: after
...
- [x] 2.2 Implement in `ConfirmForm`: record an ordered list of navigable field keys + the focus widget
...
**Quality Gate:**
- [x] ConfirmForm focus/count/selection tests pass under real Tk and skip cleanly with no display.

## Phase 3: ReviewApp keyboard bindings & exception-first load

- [x] 3.1 Add fail-first tests: a real-Tk test asserts the window binds the documented shortcuts and that
...
- [x] 3.2 Implement in `ReviewApp`: a `_bind_review_shortcuts()` called from `_build_ui`; a
...
**Quality Gate:**
- [x] App binding + exception-first-load tests pass headless (fakes) and under real Tk.

## Phase 4: Integration, docs & verification

- [x] 4.1 CHANGELOG `[Unreleased]` `### Added` entry referencing #42 and #43; README correction-workflow
  note listing the shortcuts (no new CLI subcommand → CLI help unchanged).
- [x] 4.2 `python -W error -m pytest -q` and `python -m policy_check --repo .` green.
- [x] 4.3 Behavior verified by the real-Tk automated suite (Tk available; tests ran, not skipped):
  focus-first-flagged, next/prev-flagged cycling, de-emphasis/count, digit/space option entry, and the
  binding registration are all exercised under a real Tk root. NOTE: a separate interactive operator GUI
  session (live workbook write + camera) was NOT run in this environment — recommended as a pre-release
  smoke check.
- [x] 4.4 Base OpenSpec spec (`openspec/specs/record-confirmation/spec.md`) synced on archive.

**Quality Gate:**
- [x] Full suite (598 passed, 5 skipped) + policy (16 pass / 0 fail) green; shortcuts documented; manual
  interactive run deferred (covered by real-Tk automated tests).

## Completion Checklist

- [x] All phases complete and quality gates green
- [x] CHANGELOG `[Unreleased]`, README, and PR-template checklist done
- [x] Ready for `/openspec-archive improve-review-keyboard-flow`
