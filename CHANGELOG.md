...
- 可攜 release 打包（release stage）：新增 `build/build_vlm_runtime.py`，把本機 Ollama runtime ＋**僅預設模型**
  ...
  解析 runtime（env→user→bundle）並在需要時起 bundled `ollama serve`；app 在 `OCR_BACKEND=vision` 時自動起
  bundled server 並走 vision 預填。實測 bundle 以自帶模型在獨立 port 辨識成功（identity→patient），完全不依賴
  系統 Ollama。模型/runtime 不進 git。
