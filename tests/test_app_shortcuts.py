...
def test_roster_entry_points_inert_in_scan_mode(monkeypatch):
    app = _headless_app()
    app._review_mode = "scan"
    applied = []
    monkeypatch.setattr(app, "_apply_roster_choice", lambda name: applied.append(name))

    app._roster_listbox = _FakeListbox(["王小明"], selection=(0,))
    assert app._on_roster_commit() == "break"
    assert applied == []  # roster commit can't mutate a record during capture
    assert app._on_focus_roster_key() == "break"
    assert app._roster_listbox.focused is False  # F8 doesn't grab the roster in scan mode
